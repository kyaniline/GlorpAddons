package com.glorpaddons.farming

import net.minecraft.client.MinecraftClient
import net.minecraft.component.DataComponentTypes
import net.minecraft.component.type.ProfileComponent
import net.minecraft.entity.Entity
import net.minecraft.entity.decoration.ArmorStandEntity
import net.minecraft.entity.EquipmentSlot
import net.minecraft.item.Items
import java.util.Collections
import java.util.WeakHashMap

object PestHighlighter {

    /** Glow color: dark red matching Skyblocker (0xB62F00) with full alpha. */
    const val GLOW_COLOR = 0xFFB62F00.toInt()

    /**
     * Base64-encoded texture values for all known Hypixel Skyblock pest heads.
     * Sourced from Skyblocker's HeadTextures.java.
     */
    private val PEST_HEADS = setOf(
        // Beetle
        "",
        // Cricket
        "",
        // Dragonfly
        "",
        // Earthworm (head)
        "",
        // Earthworm (tail)
        "",
        // Field Mouse
        "",
        // Firefly
        "",
        // Firefly (flash)
        "",
        // Fly
        "",
        // Locust
        "",
        // Mite
        "",
        // Mosquito
        "",
        // Moth
        "",
        // Praying Mantis
        "",
        // Rat
        "",
        // Slug
        ""
    )

    /** Set of entity IDs currently identified as pests. */
    private val pestEntityIds = mutableSetOf<Int>()

    @JvmStatic
    fun isPest(entity: Entity): Boolean = entity.id in pestEntityIds

    fun tick(client: MinecraftClient) {
        if (!FarmingConfigManager.config.pestHighlightEnabled) {
            if (pestEntityIds.isNotEmpty()) pestEntityIds.clear()
            return
        }

        val world = client.world ?: run { pestEntityIds.clear(); return }
        val player = client.player ?: run { pestEntityIds.clear(); return }

        val newIds = mutableSetOf<Int>()
        for (entity in world.getEntities()) {
            if (entity !is ArmorStandEntity) continue
            if (entity.squaredDistanceTo(player) > 1024.0) continue // 32-block range
            if (checkPestTexture(entity)) newIds.add(entity.id)
        }
        pestEntityIds.clear()
        pestEntityIds.addAll(newIds)
    }

    private fun checkPestTexture(entity: ArmorStandEntity): Boolean {
        val helmet = entity.getEquippedStack(EquipmentSlot.HEAD)
        if (helmet.isEmpty || !helmet.isOf(Items.PLAYER_HEAD)) return false
        val profileComp = helmet.get(DataComponentTypes.PROFILE) ?: return false
        val gameProfile = profileComp.gameProfile ?: return false
        for (prop in gameProfile.properties.get("textures")) {
            if (prop.value() in PEST_HEADS) return true
        }
        return false
    }
}
